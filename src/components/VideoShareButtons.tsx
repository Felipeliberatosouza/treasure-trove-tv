...
const VideoShareButtons = ({ videoTitle, videoUrl }: VideoShareButtonsProps) => {
  const encodedUrl = encodeURIComponent(videoUrl);
  const encodedText = encodeURIComponent(`Confira este vídeo: ${videoTitle}`);
        ...
        <PopoverContent className="w-auto p-2" align="start">
          <div className="flex flex-col gap-1">
            {shareLinks.map((link) => (
              ...
              >
                <span className={`flex items-center justify-center h-8 w-8 rounded-full ${link.color}`}>
                  {link.icon}
                </span>
                <span>{link.name}</span>
              </button>
